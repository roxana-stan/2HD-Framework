...
public class MaxMinEdgeCloudDatacenterBroker extends DefaultEdgeCloudDatacenterBroker {
	...
	public MaxMinEdgeCloudDatacenterBroker(String name) throws Exception {
		super(name);
	}
	...
	protected void submitCloudlets() {
		...
		/* MaxMin algorithm: do while there are unallocated tasks. */
		while (handledCloudlets.size() != taskCount) {
			...
			for (int i = 0; i < taskCount; ++i) {
				...
				/* Select the task having the maximum completion time among all unassigned tasks. */
				if (minResourceCompletionTime > maxCompletionTime) {
					maxCompletionTime = minResourceCompletionTime;
					taskIdx = i;
			...
			readyTimeArray[resourceIdx] = maxCompletionTime;
